...
class YonduBinPickingEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        object1_pos = (0.2, 1.2, self._drop_height)
        ...
        object2_pos = (0.2, 1.2, self._drop_height)
        ...
        new_pos = (0.2, 1.2, self._drop_height)
        ...
        cart_bin_cfg = copy.deepcopy(self.cfg.cart_bin_cfg)
        new_pos = (0.45, 0.05, 1.05)
        cart_bin_cfg.spawn.scale = (0.4, 0.4, 0.4)
        cart_bin_cfg.init_state.pos = new_pos
        cart_bin_cfg.init_state.rot = (0.707, 0.0, 0.0, 0.707)
        prim_path = "/World/envs/env_.*/CartBin"
        cart_bin_cfg.prim_path = prim_path
        self._object[3] = RigidObject(cart_bin_cfg)
    ...
    def _object_reached_goal(self):
        ...
        reached = self._within_bin(object_curr_pos, basket_pos)
        return reached
    ...
    def _within_bin(self, obj_pos, basket_pos):
        result = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ith = 0
        for obj_pos_ith, basket_pos_ith in zip(obj_pos, basket_pos):
            result_ith = ((obj_pos_ith[0] > basket_pos_ith[0] - 0.1) and (obj_pos_ith[0] < basket_pos_ith[0] + 0.1) and 
